# Pikmin 1 Multiworld Setup Guide
...
2. Acquire the most recent APWorld from the [Releases page](https://github.com/TheLynk/Archipelago/releases/latest) and place the pikmin.apworld in the custom_worlds folder of your Archipelago install
...
You will be prompted to locate your Pikmin 1 ISO the first time you do this.
   ...
   - You will ***not*** need to patch the game every time, and can simply run the `PikminClient` from the list on the right of the Archipelago Launcher
to continue later.
7. Open Dolphin and from Dolphin, open your newly patched Pikmin 1 ISO. Load all the way into a brand new save file, and pause.
...
8. In the server page, there will be a port number. Copy this port number into the top of your PikminClient if it did not populate on its own. 
   - The field should read `archipelago.gg:<port number>`
9. Click the `Connect` button at the top of the PikminClient BEFORE loading a save. You are now connected and ready to play!
   - WARNING: Loading a save before connecting to the server will probably break some things
10. To rejoin the room later, you need to open the webpage, open the PikminClient through the Archipelago Launcher, and open the patched ISO with Dolphin.
Then you can click connect on the PikminClient so long as the port matches what is shown on the webpage.
...
to locate your Pikmin 1 ISO

### Connect to the client

When the ISO patched, the Pikmin 1 client (PikminClient) should have also automatically launched in
...
1. Open Dolphin and from Dolphin, open your newly patched Pikmin 1 ISO
2. In the server page, there will be a port number. Copy this port number into the top of your PikminClient. 
   - The field should read `archipelago.gg:<port number>`
3. Once you have loaded into the game, the client should log that Dolphin has been connected. Click the `Connect` button
at the top of the PikminClient. If the port number is correct, you are now connected and ready to play!
